...
const employeesBonus = (arr) => {
    arr.forEach((element, index) => {
        let salary = parseInt(arr[index].salary);
        if (element.workHours > 8) {
            arr[index].salary = (salary + 100).toString() + "$";
        }
        else {
            arr[index].salary = (salary + 50).toString() + "$";
        }
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    let maxMouse = max(mouseArray);
    let maxKeyBoard = max(keyBoardArray);
    let cost = maxKeyBoard + maxMouse;

    let flag;

    while (cost > budget) {
        (maxMouse > maxKeyBoard ? removeMax(mouseArray) : removeMax(keyBoardArray));

        maxMouse = max(mouseArray);
        maxKeyBoard = max(keyBoardArray);
        cost = maxKeyBoard + maxMouse;
