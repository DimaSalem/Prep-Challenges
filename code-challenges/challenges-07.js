...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1 & (arr[i].firstName != null | arr[i].lastName != null)) {
            arr2.push({ fullName: getFullName(arr[i]), tech: arr[i].tech });
        }
    }
...
const applicationsStatics = (arr) => {
    let numPython_devs = 0, numJavaScript_devs = 0, numDotNet_devs = 0, numJava_devs = 0, numTotalApplicants = 0, numRejectedApplicants = 0;

    numTotalApplicants = arr.length;
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1 & (arr[i].firstName != null | arr[i].lastName != null)) {
            switch (arr[i].tech) {
                case "Python":
            ...
            numRejectedApplicants++;
        }
    }

    return {
        python_devs: numPython_devs, javaScript_devs: numJavaScript_devs, dotNet_devs: numDotNet_devs,
        java_devs: numJava_devs, totalApplicants: numTotalApplicants, rejectedApplicants: numRejectedApplicants
    }
